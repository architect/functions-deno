...
import {middleware as arcHttpMiddleware} from '../../../../../src/http/index.js'
...
let run = async (response, request) => {
  let fn = () => response
  let handler = arcHttpSync(fn)
  return handler(request)
}

test('Set up env', t => {
  t.plan(4)
  t.ok(arcHttpSync, 'Loaded HTTP async')
  t.ok(arcHttpMiddleware, 'Loaded HTTP middleware alias')
